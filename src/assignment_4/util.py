...
def read_json_without_schema(spark, json_path):
    return spark.read.json(json_path)

def explode_data(df, nested_column):
    return df.select(explode(col(nested_column)).alias("exploded_column"))

def filter_data(df, filter_column, condition_value):
    return df.filter(col(filter_column) == condition_value)
