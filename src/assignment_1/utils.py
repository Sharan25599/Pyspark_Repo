...
def upgraded_B_to_E(df):
    return (df.alias("p1").join(df.alias("p2"), (col("p1.customer") == col("p2.customer")) & (col("p1.product_model") == "B") & (col("p2.product_model") == "E")) .select("p1.customer").distinct())
...
def bought_all_models(purchase_data_df, product_data_df):
    ...
    return (purchase_data_df.groupBy("customer").agg(countDistinct("product_model").alias("distinct_models")).filter(col("distinct_models") == distinct_models).select("customer"))
